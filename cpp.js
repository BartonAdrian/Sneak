...
var STATS = {
    'NumOfColumns': 4,
    'Impressions': { 'Column': 3, 'Color': 'red', 'AlertRange': 'impressions_alert' },
    'Clicks': { 'Column': 4, 'Color': 'orange', 'AlertRange': 'clicks_alert' },
    'Conversions': { 'Column': 5, 'Color': 'dark yellow 2', 'AlertRange': 'conversions_alert' },
...
function main() {
    ...
    Logger.log('Using spreadsheet - %s.', 'list.xlsx');
    var spreadsheet = validateAndGetSpreadsheet('list.xlsx');
...
function processAccount(account, spreadsheet, startingRow) {
    ...
    function generateAlert(field, emailAlertText) {
        ...
        if ((-1 != validWhite.indexOf(bg[0])) &&
            ...
            bgRange[0].setBackground([
                [STATS[field]['Color']]
            ]);
            bgRange[1].setBackground([
                [STATS[field]['Color']]
            ]);
    ...
    var dataRows = [
        [
            account.getCustomerId().getMccAccount, todayStats.Impressions, todayStats.Clicks,
...
var SheetUtil = (function() {
    ...
    var setupData = function(spreadsheet) {
        ...
        var getThresholdFor = function(field) {
            thresholds[field] = parseField(spreadsheet.getRangeByName(field).getValue());
        };
...
function addRow(row, previous, coefficient) {
    ...
    return {
        ...
        Conversions: parseInt(row['Conversions']) * coefficient + previous.Conversions,
